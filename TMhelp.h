...
int readFile(const char*path, char *buf, int amount_to_read, off_t offset){
	...
	if(!node->isFile){
		...
		return -1;					//trying to read a directory
	}

	//char* plist = node->inode.plist;

	for(int i = startingPoint; i < 128 && node->inode->plist[i] != -1; i++){

		if(amount_read != amount_to_read){

			for( char c = node->inode.plist[i]; c != '\0'; node->inode.plist[i].p++){
...
/*
int writeFile(const char* path, int content_length, (void*) content, size_t offset){

	struct treeNode* node = findNode(path);
	int new_fileSize = data_length + offset;
